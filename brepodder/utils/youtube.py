...
def get_total_time(episode):
    ...
    except (json.JSONDecodeError, KeyError, TypeError, ValueError, requests.exceptions.RequestException) as e:
        logger.warning(f"Failed to get total time for video: {e}")
        return 0
    ...
    if vid is not None:
        ...
        def find_urls(old_page, new_page):
            ...
            if 'reason' in x:
                ...
                error_message = remove_html_tags(x['reason'][0])
                ...
                if 'reason' in playabilityStatus:
                    error_message = remove_html_tags(playabilityStatus['reason'])
                    ...
                    if allow_partial and 'streamingData' in player_response and 'hlsManifestUrl' in player_response['streamingData']:
                        r = urlopen(player_response['streamingData']['hlsManifestUrl'])
                        if not r.ok:
...
def get_cover(url, feed_data=None):
    if 'youtube.com' in url:
        ...
        try:
            ...
            r = urlopen(channel_url)
            if not r.ok:
                raise YouTubeError('Youtube "%s": %d %s' % (url, r.status_code, r.reason))
            html_data = response_text(r)
