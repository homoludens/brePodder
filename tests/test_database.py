...
class TestDBOperationWithInMemoryDB:
    ...
    def test_insert_channel(self, db_op, sample_channel):
        ...
        db_op.insertChannel(sample_channel)
        
        result = db_op.getAllChannels()
        assert len(result) == 1
    ...
    def test_get_channel_by_id_not_found(self, db_op):
        ...
        result = db_op.getChannelById(999)
    ...
    def test_get_channel_by_title(self, db_op, sample_channel):
        ...
        result = db_op.getChannelByTitle("Test Podcast")
    ...
    def test_get_channel_by_title_not_found(self, db_op):
        ...
        result = db_op.getChannelByTitle("Nonexistent Podcast")
        assert result is None
    ...
    def test_get_channel_by_link(self, db_op, sample_channel):
        ...
        result = db_op.getChannelByLink("https://example.com/feed.xml")
    ...
    def test_delete_channel(self, db_op, sample_channel):
        ...
        db_op.insertChannel(sample_channel)
        assert len(db_op.getAllChannels()) == 1
        
        db_op.deleteChannel("Test Podcast")
        
        assert len(db_op.getAllChannels()) == 0

    def test_insert_episode(self, db_op, sample_channel, sample_episode):
        """Should insert an episode into the database."""
        db_op.insertChannel(sample_channel)
        db_op.insertEpisode(sample_episode)
        
        result = db_op.getChannelEpisodes("Test Podcast")
        assert len(result) == 1
    ...
    def test_update_episode(self, db_op, sample_channel, sample_episode):
        ...
        db_op.updateEpisode(("/path/to/local.mp3", "downloaded", 1))
        
        episode = db_op.getEpisodeByTitle("Test Episode Title")
    ...
    def test_delete_all_episodes(self, db_op, sample_channel, sample_episode):
        ...
        assert len(db_op.getChannelEpisodes("Test Podcast")) == 2
        
        db_op.deleteAllEpisodes("Test Podcast")
        
        assert len(db_op.getChannelEpisodes("Test Podcast")) == 0
    ...
    def test_get_latest_episodes(self, db_op, sample_channel, sample_episode):
        ...
        result = db_op.getLatestEpisodes()
    ...
    def test_get_latest_downloads(self, db_op, sample_channel):
        ...
        result = db_op.getLatestDownloads()
    ...
    def test_insert_folder(self, db_op):
        ...
        result = db_op.insertFolder("My Podcasts")
        
        assert result is True
        folders = db_op.getAllFolders()
        assert len(folders) == 1
        assert folders[0]["title"] == "My Podcasts"

    def test_get_all_folders(self, db_op):
        """Should get all folders ordered by title."""
        db_op.insertFolder("Zebra Folder")
        db_op.insertFolder("Alpha Folder")
        db_op.insertFolder("Middle Folder")
    ...
    def test_delete_taxonomy(self, db_op):
        ...
        db_op.insertFolder("To Delete")
        assert len(db_op.getAllFolders()) == 1
        
        db_op.deleteTaxonomy("To Delete")
        
        assert len(db_op.getAllFolders()) == 0

    def test_is_folder(self, db_op):
        """Should return True for existing folder."""
        db_op.insertFolder("Test Folder")
    ...
    def test_add_channel_to_folder(self, db_op, sample_channel):
        ...
        db_op.insertChannel(sample_channel)
        db_op.insertFolder("My Folder")
        
        db_op.addChannelToFolder("Test Podcast", "My Folder")
        
        channels = db_op.getFolderChannels(1)
    ...
    def test_add_channel_to_no_folder(self, db_op, sample_channel):
        ...
        db_op.addChannelToFolder("Test Podcast", None)
    ...
    def test_get_all_channels_without_folder(self, db_op, sample_channel):
        ...
        db_op.insertFolder("Folder")
        db_op.addChannelToFolder("Foldered Podcast", "Folder")
        
        result = db_op.getAllChannelsWOFolder()
...
class TestPlaylist:
    ...
    def test_is_in_playlist(self, db_op, sample_channel, sample_episode):
        ...
        assert db_op.isInPlaylist(1) is False
    ...
    def test_clear_playlist(self, db_op, sample_channel, sample_episode):
        ...
        db_op.addToPlaylist(1)
        db_op.addToPlaylist(2)
        
        db_op.clearPlaylist()
        
        assert db_op.isInPlaylist(1) is False
        assert db_op.isInPlaylist(2) is False
    ...
    def test_playlist_limit(self, db_op, sample_channel):
        ...
        # Add first 50
        for i in range(1, 51):
            db_op.addToPlaylist(i)
        
        # 51st should fail
        result = db_op.addToPlaylist(51)
